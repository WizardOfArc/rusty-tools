...
fn send_to_slack(message: &str, user: &str) -> SlackResult<()> {
    ...
    let slack = Slack::new(webhook_url.as_str()).map_err(|_| SlackError::UnableToMakeSlackClient)?;
    let payload = PayloadBuilder::new()
        .text(message)
        .username(BOT_NAME)
        .channel(user)
        .icon_emoji(ICON_EMOJI)
        .build().map_err(|_| SlackError::UnableToBuildPayload)?;
    slack.send(&payload).map_err(|_| SlackError::UnableToSendMessage)
}
